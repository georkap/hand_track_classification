...
class MF_UNIT(nn.Module):
    ...
    def __init__(self, num_in, num_mid, num_out, g=1, stride=(1,1), first_block=False):
        super(MF_UNIT, self).__init__()
        num_ix = int(num_mid/4)
        # kt,pt = (3,1) if use_3d else (1,0)
        # prepare input
        self.conv_i1 =     BN_AC_CONV2D(num_in=num_in,  num_filter=num_ix,  kernel=(1,1), pad=(0,0))
        self.conv_i2 =     BN_AC_CONV2D(num_in=num_ix,  num_filter=num_in,  kernel=(1,1), pad=(0,0))
        # main part
        self.conv_m1 =     BN_AC_CONV2D(num_in=num_in,  num_filter=num_mid, kernel=(3,3), pad=(1,1), stride=stride, g=g)
        if first_block:
            self.conv_m2 = BN_AC_CONV2D(num_in=num_mid, num_filter=num_out, kernel=(1,1), pad=(0,0))
        else:
            self.conv_m2 = BN_AC_CONV2D(num_in=num_mid, num_filter=num_out, kernel=(3,3), pad=(1,1), g=g)
        # adapter
        if first_block:
            self.conv_w1 = BN_AC_CONV2D(num_in=num_in,  num_filter=num_out, kernel=(1,1), pad=(0,0), stride=stride)
...
class MFNET_2D(nn.Module):
    ...
    def __init__(self, num_classes, pretrained=False, **kwargs):
        ...
        self.globalpool = nn.Sequential(OrderedDict([
                        ('avg', nn.AvgPool2d(kernel_size=(7,7),  stride=(1,1))),
...
if __name__ == "__main__":
    ...
    # ---------
    net = MFNET_2D(num_classes=1000, pretrained=False)
    test_weights = r"D:\Code\hand_track_classification\models\MFNet2D_ImageNet1k-0000.pth"
